...
public class SpreadSheet extends JFrame {
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if (tok.substring(0,3).equalsIgnoreCase("AVG")) {
            	tok = avg(tok);
            }
            else  {          	
    ...
    //Start my code here
    // Formula Validation Method
    public boolean validFormula(String formula){
    
    	String leftParen = (formula.substring(3,4));
    	String colin = (formula.substring(6,7));
    	String rightParen = (formula.substring(9));
   ...
   public Double sum(int start, int end, int column, boolean row) {
    	double sum = 0;
        for (int i=start-1; i < end; i++) {
    		int col = column - 'A';
    		String thisCellVal = row ? this.getCellText(i, col):this.getCellText(col, (column-i));
    		if (thisCellVal.length()>0) {
    			sum += Double.parseDouble(thisCellVal);
    		}
        }
    	return sum;
    }
   ...
   // Average Method
    public String avg(String formula){
       	//Check to see if it is a valid formula with validFormula method
    	if (!validFormula(formula)) {
    		return null;
    	}
    	
    	//strip out formula, then get starting and ending columns and rows
    	String parse[] = formula.substring(4,9).split(":");
    	//columns
    	char col1 = parse[0].charAt(0);
    	char col2 = parse[1].charAt(0);
    	//rows
    	int row1 = parse[0].charAt(1);
    	int row2 = parse[1].charAt(1);
    	// put smallest column and row first to make processing loop easier
    	if (col1>col2){
    		char temp = col1;
    		col1 = col2;
    		col2 = temp;
    	...
    	//determine if column or row summation and use the sum method to get total for row or column
        double sumTot = row1!=row2  ? sum(row1,row2,col1,true) : sum(col1,col2,col1,false) ;
        double avg = row1!=row2 ? sumTot /(row2-row1+1) : sumTot /(col2-col1+1) ;
